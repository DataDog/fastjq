...
- `sort` (sort by jq type ordering: null < false < true < numbers < strings < arrays < objects; Tier 2)
- `sort_by(f)` (sort by key function; supports multi-key: `sort_by(.a, .b)`; Tier 2)
- `unique`, `unique_by(f)` (deduplicate after sort; Tier 2)
- `group_by(f)` (sort then group equal keys into sub-arrays; Tier 2)
- `transpose` (matrix transpose; null-pads short rows; Tier 2)
