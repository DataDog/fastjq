...
- **`paths` is a parity-first exception on `codex/jq-parity`**: the current implementation unlocks official jq-suite coverage but allocates on its traversal path. This is intentionally documented as a branch tradeoff while the parity goal takes precedence over the usual zero-alloc target.
