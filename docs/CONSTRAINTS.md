...
- **Supported operations**: identity (`.`), field access (`.foo`, `.foo.bar`), array indexing (`.[0]`, `.[-1]`, `.[i,j]`), slicing (`.[n:m]`, `.[:m]`, `.[n:]`, `.[n:m]?`), deletion (`del(.foo)`, `del(.[0])`, `del(.[i,j])`, `del(.[n:m])`, `delpaths(paths)`), iteration (`.[]`), object construction (`{name}`, `{a: .foo}`, `{a: .items[]}`), array construction (`[.foo, .bar]`), variable binding (`expr as $x | body`), `reduce gen as $x (init; update)`, `foreach gen as $x (init; update; extract?)`, `map(expr)`, `add`, `expr + expr` (including object merge), `expr - expr`, unary negation (`-expr`), `expr * expr` (including `string * n`, `n * string`, `object * object` recursive merge), `expr / expr`, `expr % expr`, `flatten`/`flatten(n)`, `split("s")`, `join("s")`, `min`/`max`, `min_by(f)`/`max_by(f)`, `sort`, `sort_by(f)`, `unique`, `unique_by(f)`, `group_by(f)`, `transpose`, `to_entries`, `from_entries`, `keys`, `keys_unsorted`, `paths`, `paths(filter)`, `path(expr)`, `getpath(path)`, `setpath(path; value)`, `any`/`any(expr)`/`any(gen; cond)`, `all`/`all(expr)`/`all(gen; cond)`, `first`/`first(expr)`, `last`/`last(expr)`, `limit(n; expr)`, `skip(n; expr)`, `nth(n; gen)`, `isempty(expr)`, string interpolation (`"\(expr)"`), pipe (`expr | expr`), grouping (`(expr)`), literals (`null`, `true`, `false`, `"string"`, `123`), comparison (`==`, `!=`, `<`, `<=`, `>`, `>=`) with multi-output support, boolean (`and`, `or`, `not`), `has("key")`, `length`, `abs`, `ascii_downcase`, `ascii_upcase`, `startswith("s")`, `endswith("s")`, `trim`, `ltrim`, `rtrim`, `ltrimstr("s")`, `rtrimstr("s")`, `if-then-elif-...-else-end`, `empty`, select (`select(cond)`), alternative (`//`), optional (`.foo?`), type (`type`), `try`/`try-catch`, `error`/`error(expr)`, `tojson`/`@json`, `fromjson`, `tostring`/`@text`, `tonumber`, `toboolean`, `@base64`, `@base64d`, `@uri`, `@urid`, `@html`, `@csv`, `@tsv`, `@sh`, `floor`, `ceil`, `round`, `nearbyint`, `contains(val)`, `inside(val)`, `index(s)`, `rindex(s)`, `indices(s)`, `sqrt`, `fabs`, `atan` (1-arg), `log`, `log2`, `log10`, `exp`, `exp2`, `exp10`, `cbrt`, `logb`, `sin`, `cos`, `tan`, `asin`, `acos`, `tgamma`, `lgamma`, `j0`, `j1`, `test(re)`/`test(re; flags)`, `match(re)`/`match(re; flags)`, `capture(re)`/`capture(re; flags)`, `scan(re)`/`scan(re; flags)`, `sub(re; s)`, `gsub(re; s)`
