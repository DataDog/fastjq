...
- **`paths` and `path(expr)` are parity-first exceptions on `codex/jq-parity`**: the current implementations unlock official jq-suite coverage but allocate on the path-building call path (`paths`: `17 allocs/op` on the small benchmark, `path(.field_0)`: `6 allocs/op`). This is intentionally documented as a branch tradeoff while the parity goal takes precedence over the usual zero-alloc target.
