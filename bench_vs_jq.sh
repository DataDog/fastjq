...
add_bench "field access"            ".field_50"                                         "$LARGE_FILE" "large"
add_bench "delete field"            "del(.field_2)"                                     "$SMALL_FILE" "small"
add_bench "delete field"            "del(.field_50)"                                    "$LARGE_FILE" "large"
add_bench "object construction"     "{field_0, field_2}"                                "$SMALL_FILE" "small"
add_bench "object construction"     "{field_0, field_50}"                               "$LARGE_FILE" "large"
add_bench "select (all match)"      'select(.field_2 == "xxxxxxxxxx")'                  "$SMALL_FILE" "small"
add_bench "select (all match)"      'select(.field_199 | startswith("x"))'              "$LARGE_FILE" "large"
add_bench "select (none match)"     'select(.field_2 == "nope")'                        "$SMALL_FILE" "small"
add_bench "select (none match)"     'select(.field_50 == "nope")'                       "$LARGE_FILE" "large"
add_bench "alternative"             '.field_2 // "default"'                             "$SMALL_FILE" "small"
add_bench "alternative"             '.missing // "default"'                             "$LARGE_FILE" "large"
add_bench "case-insens. select"     'select(.field_2 | ascii_downcase == "xxxxxxxxxx")' "$SMALL_FILE" "small"
add_bench "case-insens. select"     'select(.field_50 | ascii_downcase | startswith("x"))' "$LARGE_FILE" "large"
add_bench "prefix filter"           'select(.field_2 | startswith("xxxx"))'             "$SMALL_FILE" "small"
add_bench "prefix filter"           'select(.field_50 | startswith("xxxx"))'            "$LARGE_FILE" "large"
add_bench "field existence"         'select(has("field_2"))'                            "$SMALL_FILE" "small"
add_bench "field existence"         'select(has("field_50"))'                           "$LARGE_FILE" "large"
add_bench "to_entries"              "to_entries"                                        "$SMALL_FILE" "small"
add_bench "to_entries"              "to_entries"                                        "$LARGE_FILE" "large"
add_bench "keys"                    "keys_unsorted"                                     "$SMALL_FILE" "small"
add_bench "keys"                    "keys_unsorted"                                     "$LARGE_FILE" "large"
