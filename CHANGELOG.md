...
## [Unreleased] — parser breadth, jq parity majority, and parity builtin sweep
...
- Added benchmark coverage for the new public surface: variable binding, `abs`, `toboolean`, `trim`, `ltrim`, `rtrim`, `keys`, and `skip`.
...
- Moved the official jq-suite branch coverage from `356/751` passing to `405/751` passing while keeping `0` active jq-suite failures.
...
- Regenerated `docs/BENCHMARKS.md` and kept the benchmark table current while parity work expanded parser coverage.
